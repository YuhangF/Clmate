...
public class Climate extends AppCompatActivity implements View.OnClickListener{
    TextView Textshow;
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try{
                    ...
                    String responseData = response.body().string();//从网页中获得String类型信息
                    Log.d("data is", responseData);
                    showResponse(responseData);       //将获得的字符串信息展示在Textshow
                }catch (Exception e){
                    e.printStackTrace();
                }
            }
    ...
    private void parseJSONWithFastJSON(String jsonData){
        if(jsonData.length()<100){
            ...
            Climate.this.setResult(RESULT_OK,getIntent());
            Climate.this.finish();
        }
        else {
